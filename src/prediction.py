...
MODEL_PATH=os.path.join(ROOT,"models","student_model.pkl")
class StudentPerformancePredictor:
    ...
    def predict(self,study_hours,attendance,previous_marks,assignment_score,sleep_hours):
        row=prepare_input(study_hours,attendance,previous_marks,assignment_score,sleep_hours); 
        pred=int(self.model.predict(row)[0]); 
        conf=float(max(self.model.predict_proba(row)[0]))*100
        return {"result":"PASS" 
        if pred==1 
        else "FAIL","confidence":conf}
def number(prompt):
    while True:
